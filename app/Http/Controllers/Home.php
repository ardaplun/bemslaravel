...
class Home extends Controller
{
    ...
    public function usageprofile()
    {
      ...
      foreach ($buildings as $building) {
        ...
        $daysinmonth  = intval(cal_days_in_month(CAL_GREGORIAN, $thsmnth, $thsyr));
        for ($i=1; $i <= $daysinmonth; $i++) {
          $tmpday1          = Energy::getEnergy($where[3],'date(time) = ?',Carbon::createFromDate($thsyr, $thsmnth, $i)->toDateString());
          $tmpday2          = Energy::getEnergy($where[3],'date(time) = ?',Carbon::createFromDate($thsyr, $thsmnth, ($i-1))->toDateString());
          $data['dtmnth'][] = round(($tmpday1-$tmpday2)/1000,2);
        }
        // <-- end of calculating month data -->
